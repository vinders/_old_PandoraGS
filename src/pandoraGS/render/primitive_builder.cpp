...
// GPU operations
gpucmd_t PrimitiveBuilder::s_gpuCommand;
unsigned long PrimitiveBuilder::s_gpuMemCache[256]; // memory cache
long PrimitiveBuilder::s_gpuDataCount;              // data set length
long PrimitiveBuilder::s_gpuDataProcessed;          // current number of values cached
...
bool PrimitiveBuilder::processDisplayData(loadmode_t& writeModeRef, unsigned long* pDwMem, int size, unsigned long* pDest, int* pI)
{
...
void cmBlankFill(unsigned char* pData)
{
    ...
    // coords
    wtile_t zone;//...
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[1], zone.x, zone.y);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[2], zone.width, zone.height);
...
void cmTexPage(unsigned char* pData)
{
    ...
    bool is24bit = ((primData & 0x200uL) != 0uL);//...
    bool isDrawingToDisplay = ((primData & 0x400uL) != 0uL);//...
    bool isTextureDisabled = (((primData & 0x800uL) != 0uL) && ((primData & 0x1uL) != 0uL));//...
    bool isFlippedX = ((primData & 0x1000uL) != 0uL);//...
    bool isFlippedY = ((primData & 0x2000uL) != 0uL);//...
...
void cmMaskBit(unsigned char* pData)
{
    ...
    bool isBit15Forced = ((primData & 0x1uL) != 0uL);//...
    bool isBit15Checked = ((primData & 0x2uL) != 0uL);//...
...
void cmTriangle(unsigned char* pData)
{
    ...
    // vertices
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[1], g_curTriangle.x0, g_curTriangle.y0);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[2], g_curTriangle.x1, g_curTriangle.y1);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[3], g_curTriangle.x2, g_curTriangle.y2);
    //color = pPrimData[0];
    bool isOpaque = PrimitiveBuilder::isShapeOpaque(*pPrimData);//...
...
void cmTriangleS(unsigned char* pData)
{
    ...
    // vertices
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[1], g_curTriangle.x0, g_curTriangle.y0);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[3], g_curTriangle.x1, g_curTriangle.y1);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[5], g_curTriangle.x2, g_curTriangle.y2);
    //color = pPrimData[0];
    //color1 = pPrimData[2];
    //color2 = pPrimData[4];
    bool isOpaque = PrimitiveBuilder::isShapeOpaque(*pPrimData);//...
...
void cmTriangleTx(unsigned char* pData)
{
    ...
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[5], g_curTriangle.x2, g_curTriangle.y2);
    // texture coords
    PrimitiveBuilder::extractPrimitivePos16(pPrimData[2], g_curTriangleTxCoords.x0, g_curTriangleTxCoords.y0);
    PrimitiveBuilder::extractPrimitivePos16(pPrimData[4], g_curTriangleTxCoords.x1, g_curTriangleTxCoords.y1);
    PrimitiveBuilder::extractPrimitivePos16(pPrimData[6], g_curTriangleTxCoords.x2, g_curTriangleTxCoords.y2);
    ...
    if (PrimitiveBuilder::isShapeBlended(*pPrimData))
    {
        //color = pPrimData[0];
    }
    else
...
void cmTriangleSTx(unsigned char* pData)
{
    ...
    // vertices
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[1], g_curTriangle.x0, g_curTriangle.y0);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[4], g_curTriangle.x1, g_curTriangle.y1);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[7], g_curTriangle.x2, g_curTriangle.y2);
    // texture coords
    PrimitiveBuilder::extractPrimitivePos16(pPrimData[2], g_curTriangleTxCoords.x0, g_curTriangleTxCoords.y0);
    PrimitiveBuilder::extractPrimitivePos16(pPrimData[5], g_curTriangleTxCoords.x1, g_curTriangleTxCoords.y1);
    PrimitiveBuilder::extractPrimitivePos16(pPrimData[8], g_curTriangleTxCoords.x2, g_curTriangleTxCoords.y2);
...
void cmQuad(unsigned char* pData)
{
    ...
    // vertices
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[1], g_curQuad.x0, g_curQuad.y0);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[2], g_curQuad.x1, g_curQuad.y1);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[3], g_curQuad.x2, g_curQuad.y2);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[4], g_curQuad.x3, g_curQuad.y3);
    //color = pPrimData[0];
    bool isOpaque = PrimitiveBuilder::isShapeOpaque(*pPrimData);//...
...
void cmQuadS(unsigned char* pData)
{
    ...
    // vertices
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[1], g_curQuad.x0, g_curQuad.y0);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[3], g_curQuad.x1, g_curQuad.y1);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[5], g_curQuad.x2, g_curQuad.y2);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[7], g_curQuad.x3, g_curQuad.y3);
    //color = pPrimData[0];
    //color1 = pPrimData[2];
    //color2 = pPrimData[4];
    //color2 = pPrimData[6];
...
void cmQuadTx(unsigned char* pData)
{
    ...
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[7], g_curQuad.x3, g_curQuad.y3);
    // texture coords
    PrimitiveBuilder::extractPrimitivePos16(pPrimData[2], g_curQuadTxCoords.x0, g_curQuadTxCoords.y0);
    PrimitiveBuilder::extractPrimitivePos16(pPrimData[4], g_curQuadTxCoords.x1, g_curQuadTxCoords.y1);
    PrimitiveBuilder::extractPrimitivePos16(pPrimData[6], g_curQuadTxCoords.x2, g_curQuadTxCoords.y2);
    PrimitiveBuilder::extractPrimitivePos16(pPrimData[8], g_curQuadTxCoords.x3, g_curQuadTxCoords.y3);
...
void cmQuadSTx(unsigned char* pData)
{
    ...
    // vertices
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[1], g_curQuad.x0, g_curQuad.y0);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[4], g_curQuad.x1, g_curQuad.y1);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[7], g_curQuad.x2, g_curQuad.y2);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[10], g_curQuad.x3, g_curQuad.y3);
    // texture coords
    PrimitiveBuilder::extractPrimitivePos16(pPrimData[2], g_curQuadTxCoords.x0, g_curQuadTxCoords.y0);
    PrimitiveBuilder::extractPrimitivePos16(pPrimData[5], g_curQuadTxCoords.x1, g_curQuadTxCoords.y1);
    PrimitiveBuilder::extractPrimitivePos16(pPrimData[8], g_curQuadTxCoords.x2, g_curQuadTxCoords.y2);
    PrimitiveBuilder::extractPrimitivePos16(pPrimData[11], g_curQuadTxCoords.x3, g_curQuadTxCoords.y3);
...
void cmLine(unsigned char* pData)
{
    ...
    // vertices
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[1], g_curLine.x0, g_curLine.y0);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[2], g_curLine.x1, g_curLine.y1);
    //color = pPrimData[0];
    bool isOpaque = PrimitiveBuilder::isShapeOpaque(*pPrimData);//...
...
void cmLineS(unsigned char* pData)
{
    ...
    // vertices
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[1], g_curLine.x0, g_curLine.y0);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[3], g_curLine.x1, g_curLine.y1);
    //color = pPrimData[0];
    //color1 = pPrimData[2];
    bool isOpaque = PrimitiveBuilder::isShapeOpaque(*pPrimData);//...
...
void cmPolyLine(unsigned char* pData)
{
    ...
    for (int i = 3; i <= PLINE_MAX_LEN && (*pPrimData & 0xF000F000) != 0x50005000; ++i)
    {
        ...
        PrimitiveBuilder::extractPrimitivePos32(*pPrimData, g_curLine.x1, g_curLine.y1);
        ++pPrimData;
        //... draw
    }
}
...
void cmTile(unsigned char* pData)
{
    ...
    // coords
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[1], g_curRect.x, g_curRect.y);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[2], g_curRect.width, g_curRect.height);
    //color = pPrimData[0];
    bool isOpaque = PrimitiveBuilder::isShapeOpaque(*pPrimData);//...
...
void cmSprite(unsigned char* pData)
{
    ...
    // coords
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[1], g_curRect.x, g_curRect.y);
    PrimitiveBuilder::extractPrimitivePos32(pPrimData[3], g_curRect.width, g_curRect.height);
    // texture coords
    PrimitiveBuilder::extractPrimitivePos16(pPrimData[2], g_curRectTxCoords.x, g_curRectTxCoords.y);
