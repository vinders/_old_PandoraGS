...
TEST_F(RateFactoryTest, smpteRateFactory) {
  ...
  EXPECT_EQ(1u, RateFactory::fromSmpteRate(SmpteRate::pal_25p_50i).denominator());
  EXPECT_EQ(1u, RateFactory::fromSmpteRate(SmpteRate::pal_50p).denominator());
  EXPECT_EQ(1001u, RateFactory::fromSmpteRate(SmpteRate::ntsc_30p_60i).denominator());
  EXPECT_EQ(1001u, RateFactory::fromSmpteRate(SmpteRate::ntsc_60p).denominator());
  EXPECT_EQ(1u, RateFactory::fromSmpteRate(SmpteRate::ntsc_non_drop_30p_60i).denominator());
  EXPECT_EQ(1u, RateFactory::fromSmpteRate(SmpteRate::ntsc_non_drop_60p).denominator());
  EXPECT_EQ(Rate(1, 1u), RateFactory::fromSmpteRate((SmpteRate)123456789));
}
...
TEST_F(RateFactoryTest, audioSampleRateFactory) {
  ...
  EXPECT_EQ(Rate(123456789, 1u), RateFactory::fromAudioSampleRate((AudioSampleRate)123456789));
}
