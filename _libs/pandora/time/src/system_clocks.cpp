...
# if defined(__MACH__) && !defined(CLOCK_MONOTONIC)
    ...
    double MachSystemClock::_getClockPeriod() noexcept {
      ...
      if (clock_get_res(mach_host_self(), &periodNano) == 0)
        return static_cast<double>(periodNano);
      return MachSystemClock::_defaultSystemPeriod();
    }
...
# elif defined(_POSIX_MONOTONIC_CLOCK) || defined(CLOCK_MONOTONIC)
    ...
    double MonotonicClock::_getClockPeriod() noexcept {
      ...
      if (clock_getres(CLOCK_MONOTONIC, &period) == 0)
        return (static_cast<double>(period.tv_sec) * 1000000000.0 + static_cast<double>(period.tv_nsec));
      return MonotonicClock::_defaultSystemPeriod();
    }
