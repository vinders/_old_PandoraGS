...
namespace pandora {
  namespace pattern {
    ...
    private:
      ...
      void insert(std::unique_ptr<Contract>& contract) noexcept; // Register a contract (internal usage)
    ...
    public:
      ...
      inline void clear() noexcept { this->_contracts->clear(); }
    ...
    public:
      ...
      constexpr inline bool operator==(const Contract& rhs) const { return (this->_ticket.id() == rhs._ticket.id()); } // ID never changes -> no need to lock
    ...
    inline void RegistrationCollection::insert(std::unique_ptr<Contract>& contract) noexcept {
      std::lock_guard<pandora::thread::SpinLock> guard(this->_contractsLock);
      this->_contracts.emplace(contract->ticket(), contract.get());
    }
