  ...
  namespace pattern {
    ...
    public:
      ...
      std::unique_ptr<Contract> registerFunction(function_type_move function) noexcept {
        ...
        if (function != nullptr)
          this->_functions->create(*contract, function);
        return std::move(contract);
      }
      inline std::unique_ptr<Contract> operator+=(function_type_move function) noexcept { return registerFunction(std::forward(function)); }
      
      /// @brief Cancel a subscription
      inline void unregister(Contract& contract) noexcept { contract.revoke(); }
      inline void operator-=(const Contract& contract) noexcept { unregister(contract); }
      ...
      void run(_Params&... args) const {
        ...
        for (auto& fct : lockedFunctions.value())
          fct(std::forward<_Params>(args)...);
      }
      
      /// @brief Execute each function and return results
      template <typename std::enable_if<!std::is_same<_ReturnType, void>::value>::type = 0>
      std::vector<_ReturnType> runResults(_Params&... args) const {
        std::vector<_ReturnType> results;
        auto lockedFunctions = this->_functions->data();
        for (auto& fct : lockedFunctions.value())
          results.emplace_back(fct(std::forward<_Params>(args)...));
        return results;
      }
      
    private:
      ContractProvider _contractProvider;
      std::shared_ptr<registrar> _functions = std::make_shared<registrar>();
    };
    ...
    /// @warning To use this alias, you NEED to include <thread/spin_lock.h> too.
    template <typename _ReturnType, typename ... _Params>
