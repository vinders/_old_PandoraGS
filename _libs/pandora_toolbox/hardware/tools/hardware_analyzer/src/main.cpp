...
#   ifdef _WINDOWS
      printf(" - '%S': %ux%u (work area:%ux%u) %s\n", it.attributes().id.c_str(), it.attributes().screenArea.width, it.attributes().screenArea.height, it.attributes().workArea.width, it.attributes().workArea.height, it.attributes().isPrimary ? "- primary" : " ");
#   else
      printf(" - '%d': %ux%u (work area:%ux%u) %s\n", it.attributes().id, it.attributes().screenArea.width, it.attributes().screenArea.height, it.attributes().workArea.width, it.attributes().workArea.height, it.attributes().isPrimary ? "- primary" : " ");
#   endif
  }
  printf("\n____________________________________________________________\n\n");
}
