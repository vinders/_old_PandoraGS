...
// objects state
#define ASSERT_NULL(test)                    _UTT_CHECK_TEST(ObjectVerifier::isNull(test),                 std::string("expected: nullptr ; value: not null"))
#define ASSERT_NOT_NULL(test)                _UTT_CHECK_TEST(ObjectVerifier::isNotNull(test),              std::string("expected: not null; value: nullptr"))
#define ASSERT_INSTANCE(class,test)          _UTT_CHECK_TEST(ObjectVerifier::isInstanceOf<class>(test),    std::string("not an instance of specified class"))
#define ASSERT_NOT_INSTANCE(class,test)      _UTT_CHECK_TEST(ObjectVerifier::isNotInstanceOf<class>(test), std::string("is instance of specified class"))

// strings
#define ASSERT_STR_EMPTY(test)               _UTT_CHECK_TEST(StringVerifier::isEmpty(test),        std::string("expected: \"\"; value: \")+test+std::string("\""))
#define ASSERT_STR_NOT_EMPTY(test)           _UTT_CHECK_TEST(StringVerifier::isNotEmpty(test),     std::string("expected: not empty; value: \"\""))
#define ASSERT_STR_EQ(test,val)              _UTT_CHECK_TEST(StringVerifier::isEqual(test,val),    std::string("expected: \"")+val+std::string("\"; value: \")+test+std::string("\""))
#define ASSERT_STR_NOT_EQ(test,val)          _UTT_CHECK_TEST(StringVerifier::isNotEqual(test,val), std::string("expected: != \"")+val+std::string("\"; value: \")+test+std::string("\""))
#define ASSERT_STR_START(test,val)           _UTT_CHECK_TEST(StringVerifier::startsWith(test,val), std::string("expected: \"")+val+std::string("\"[...]; value: \")+test+std::string("\""))
#define ASSERT_STR_END(test,val)             _UTT_CHECK_TEST(StringVerifier::endsWith(test,val),   std::string("expected: [...]\"")+val+std::string("\"; value: \")+test+std::string("\""))
#define ASSERT_STR_CONTAINS(test,val)        _UTT_CHECK_TEST(StringVerifier::contains(test,val),   std::string("expected: [...]\"")+val+std::string("\"[...]; value: \")+test+std::string("\""))

// arrays / hashes
#define ASSERT_ARRAY_EMPTY(test)             _UTT_CHECK_TEST(CollectionVerifier::isEmpty(test),          std::string("expected: {}; value: not empty"))
#define ASSERT_ARRAY_NOT_EMPTY(test)         _UTT_CHECK_TEST(CollectionVerifier::isNotEmpty(test),       std::string("expected: not empty; value: {}"))
#define ASSERT_ARRAY_CONTAINS(test,val)      _UTT_CHECK_TEST(CollectionVerifier::contains(test,val),     std::string("value not found in array"))
#define ASSERT_ARRAY_CONTAINS(test,key,val)  _UTT_CHECK_TEST(CollectionVerifier::contains(test,key,val), std::string("key or value not found in hash"))
#define ASSERT_ARRAY_LACKS(test,val)         _UTT_CHECK_TEST(CollectionVerifier::lacks(test,val),        std::string("excluded value is in array"))
#define ASSERT_ARRAY_LACKS(test,key,val)     _UTT_CHECK_TEST(CollectionVerifier::lacks(test,key,val),    std::string("excluded key and value is in hash"))
