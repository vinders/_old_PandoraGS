...
using namespace pandora::hardware;
  ...
  static inline std::string _toCpuVendorString(const CpuRegister128& rawValue) noexcept {
    ...
    *reinterpret_cast<uint32_t*>(vendor) = rawValue[static_cast<uint32_t>(cpuid_x86::vendorPart1().index())];
    *(reinterpret_cast<uint32_t*>(vendor) + 1) = rawValue[static_cast<uint32_t>(cpuid_x86::vendorPart2().index())];
    *(reinterpret_cast<uint32_t*>(vendor) + 2) = rawValue[static_cast<uint32_t>(cpuid_x86::vendorPart3().index())];
