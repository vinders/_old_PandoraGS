...
TEST_F(CpuSpecsTest, readExtendedCpuSpecs) {
  ...
  EXPECT_TRUE(specs.getAvailableCpuInstructionSets().size() >= 1u);
...
# ifndef __APPLE__
    EXPECT_EQ(specs.hasBaseProperty(cpuid_x86::avx()), specs.isInstructionSetSupported(CpuInstructionSet::avx));
