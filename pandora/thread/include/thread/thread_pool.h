...
namespace pandora {
  namespace thread {
    ...
    public:
      ...
      inline ThreadPool(Type&& rhs) : _poolData(std::move(rhs._poolData)) {
    ...
    protected:
      ...
      static void _runThread(std::shared_ptr<SharedPoolData> shared, uint32_t index, task_runner_type commonRunner) noexcept {
        ...
        while (sync.isRunning) {
          ...
          if (sync.isRunning) {
            ...
            catch (const std::exception& exc) { TRACE_N("ThreadPool: exception: %s", exc.what()); }
