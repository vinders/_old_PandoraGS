  ...
  namespace pattern {
    ...
    public:
      ...
      std::unique_ptr<Contract> addObserver(std::shared_ptr<observer_type> observer) {
        ...
        if (observer != nullptr)
          this->_observers->create(*contract, observer);
      ...
      void notify(_Params... args) const {
        ...
        for (auto& obs : lockedFunctions.value())
          obs->onUpdate(std::forward<_Params>(args)...);
      }
      
    private:
      ContractProvider _contractProvider;
      std::shared_ptr<registrar> _observers = std::make_shared<registrar>();
    };
