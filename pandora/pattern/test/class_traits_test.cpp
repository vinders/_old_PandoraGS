...
struct EnablerTester : public ConstructorSelector<T>, public AssignmentSelector<T> {
  ...
  EnablerTester(EnablerTester<T>&& rhs) : ConstructorSelector<T>(std::move(rhs)) { value = std::move(rhs.value); }
  EnablerTester& operator=(const EnablerTester<T>& rhs) { AssignmentSelector<T>::operator=(rhs);  value = rhs.value; }
  EnablerTester& operator=(EnablerTester<T>&& rhs) { AssignmentSelector<T>::operator=(std::move(rhs)); value = std::move(rhs.value); }
  T value;
};
