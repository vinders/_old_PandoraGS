...
void _validateClockTimes() {
  ...
  if (nsPerPeriod < 1000000.0) {
    EXPECT_GE(_ClockType::toTimePoint(ticks2), time1 - std::chrono::nanoseconds(1000000));
    EXPECT_LE(_ClockType::toTimePoint(ticks2), time2 + std::chrono::nanoseconds(5000000));
  }
  else {
    EXPECT_GE(_ClockType::toTimePoint(ticks2), time1 - std::chrono::nanoseconds(5000000));
    EXPECT_LE(_ClockType::toTimePoint(ticks2), time2 + std::chrono::nanoseconds(16000000));
  }
